...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        for i in range(n_assets):
            forwards_T_i=forward[i]*np.exp(-0.5*texp*cov_m[i,i]+np.sqrt(texp)*chol_m[i]@znorm_m)
            forwards_T.append(forwards_T_i)
        prices=np.array(forwards_T)
    else:
...
def basket_price_norm_analytic(
    ...
    cov_m = vol * cor_m * vol[:,None]
    basket_vol=np.sqrt(weights@cov_m@weights.T)
    price=normal_formula(strike,forward,basket_vol,texp,intr=intr,divr=divr,cp_sign=1)
